...
*/
var dinosaurs = [
  {name: 'tyrannosaurus', diet: 'carnivorous', weight: '7000kg', length: '12m', period: 'Late Cretaceous', roar: roar = function(){
...
const animalNames = [];
zooAnimals.forEach((zoo) => animalNames.push(`Name: ${zoo.animal_name}, Scientific: ${zoo.scientific_name}`));
...
const lowerCase = zooAnimals.map((zoo) => zoo.animal_name.toLowerCase());
...
const lowerPopulation = zooAnimals.filter((zoo) => zoo.population < 5);
...
const populationTotal = zooAnimals.reduce((acc, zoo) => acc + zoo.population,0);
